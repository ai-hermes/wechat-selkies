...
func NewPostgreSQLMigration(dsn string, wechatDB *wechatdb.DB) (*PostgreSQLMigration, error) {
	...
	log.Info().Str("dsn", maskDSN(dsn)).Msg("postgres database connected")
...
func (m *PostgreSQLMigration) Init() error {
...
func (m *PostgreSQLMigration) OptimizeDB() error {
	if m.DB == nil {
	...
	settings := []struct {
		...
		{"SET synchronous_commit = OFF", "synchronous commit"},
		{"SET maintenance_work_mem = '256MB'", "maintenance work mem"},
...
func (m *PostgreSQLMigration) Export() error {
...
func (m *PostgreSQLMigration) ExportContact() error {
...
func (m *PostgreSQLMigration) ExportChatRoom() error {
...
func (m *PostgreSQLMigration) ExportMessage() error {
	insertStmt, err := m.DB.Prepare(`
		...
		ON CONFLICT (seq, talker) DO NOTHING
	`)
	if err != nil {
